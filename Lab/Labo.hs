...
type AsignacionBool = [(String, Bool)]
...
{-
  Ejercicio: Ley de absorción
  Ejemplos: 
  abosorcion (Neg (Var "a"))
  abosorcion (Conj (Neg (Var "a")) (Var "b"))
  >abosorcion ( Conj (Var "a") (Disy (Var "a") (Var "b")) )
  >abosorcion ( Disy (Var "a") (Conj (Var "b") (Var "c")) )
-}
abosorcion :: LProp -> LProp
abosorcion a@PTrue = a
abosorcion a@PFalse = a
abosorcion a@(Var n) = a
abosorcion (Neg a) = Neg (abosorcion a)
abosorcion (Conj a (Disy b c)) = if a == b then abosorcion a else (Conj (abosorcion a) (Disy (abosorcion b) (abosorcion c)))
abosorcion (Conj a b) = Conj (abosorcion a) (abosorcion b)
abosorcion (Disy a (Conj b c)) = if a == b then abosorcion a else (Disy (abosorcion a) (Conj (abosorcion b) (abosorcion c)))
abosorcion (Disy a b) = Disy (abosorcion a) (abosorcion b)
abosorcion (Impl a b) = Impl (abosorcion a) (abosorcion b)
abosorcion (Syss a b) = Syss (abosorcion a) (abosorcion b)
...
{-|
  Ejercicio: Número de ocurrencias de la negación.
  Ejemplos:
  >num_negacion (Var "p")
  num_negacion (Disy (Var "u") (Disy (Var "w") (Var"a")))
  >num_negacion (Syss (Neg (Var "n")) (Neg (Conj PTrue (Var "m"))))
  num_negacion (Syss (Impl (Var "e") (Var "k")) (Impl (Var "j") (Conj (Var "a") (Var "l"))))
  >num_negacion (Neg (Neg (Neg (Neg (Neg (Var "s"))))))
-}
num_negacion :: LProp -> Int
num_negacion a@PTrue = 0
num_negacion a@PFalse = 0
num_negacion a@(Var n) = 0
num_negacion (Neg a) = 1 + num_negacion a
num_negacion (Conj a b) = num_negacion a + num_negacion b
num_negacion (Disy a b) = num_negacion a + num_negacion b
num_negacion (Impl a b) = num_negacion a + num_negacion b
num_negacion (Syss a b) = num_negacion a + num_negacion b
-- num_negacion _ = 0

{-|
  Ejercicio: Ocurrencias de And y Or
  Ejemplos:
  occurence_and_or (Var "p")
  occurence_and_or (Disy (Var "u") (Disy (Var "w") (Var"a")))
  occurence_and_or (Syss (Neg (Var "n")) (Neg (Conj PTrue (Var "m"))))
  occurence_and_or (Disy (Conj (Var "e") (Var "k")) (Impl (Var "j") (Conj (Var "a") (Var "l"))))
  occurence_and_or (Neg (Neg (Neg (Neg (Neg (Var "s"))))))
-}
occurence_and_or :: LProp -> Int
occurence_and_or (Neg a) = occurence_and_or a
occurence_and_or (Conj a b) = 1 + occurence_and_or a + occurence_and_or b
occurence_and_or (Disy a b) = 1 + occurence_and_or a + occurence_and_or b
occurence_and_or (Impl a b) = occurence_and_or a + occurence_and_or b
occurence_and_or (Syss a b) = occurence_and_or a + occurence_and_or b
occurence_and_or _ = 0
...
{-|
  Ejercicio: Interpretación de AND
  Ejemplos:
  interpretacion_and (Disy (Var "p") (Var "q")) [("p", 0), ("q", 1)]
  interpretacion_and (Conj (Var "p") (Var "q")) [("p", 0), ("q", 1)]
  interpretacion_and (Conj (Var "p") (Var "q")) [("p", 1), ("q", 1)]
  interpretacion_and (Disy (Impl (Var "p") (Var "q")) (Var "r")) [("p", 1), ("q", 0), ("r", 1)]
-}
-- interpretacion_and :: LProp -> AsignacionBool -> Bool
-- interpretacion_and PTrue _ = True
-- interpretacion_and PFalse _ = False
-- interpretacion_and (Var n) a = True


-- validaAsignaciones :: String -> AsignacionBool -> Int
-- validaAsignaciones n ((s,valor):xs) = if n == s then valor else validaAsignaciones n xs